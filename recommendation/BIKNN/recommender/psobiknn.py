...
class PSOBIKNN:
    ...
    def __init__(self, BIKNN, generation, swarm_size, low, high,
                 w, c1, c2, vmin, vmax, verbose):
    ...
    def predict(self, data):
        ...
        graded_swarm = self._compute_cost(swarm, data)
    ...
    def _compute_cost(self, swarm, data):
        """
        ...
        for position in swarm:
            position_B1, position_B2 = position
            self.BIKNN.B1 = position_B1
            self.BIKNN.B2 = position_B2
            pred = self.BIKNN.predict(data)
            cost = self.BIKNN.evaluate( pred, data['ratings'] )
            graded.append( self.info( cost, list(position) ) )
        
        graded = sorted(graded)
        return graded


    def _evolve(self, data, graded_swarm, velocity, pbest, gbest):
        ...
        graded_swarm_new = self._compute_cost(swarm, data)
