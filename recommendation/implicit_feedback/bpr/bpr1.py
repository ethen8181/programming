        ...
        with tf.Session() as sess:
            ...
            for iteration in loop:
                ...
                for _ in range(batch_iters):
                    ...
                    _, loss = sess.run([self._train_step, self._total_loss], feed_dict)
                    iteration_loss += loss / self.batch_size

                iteration_loss /= batch_iters
                # summary = sess.run(self._summary_op)
                # writer.add_summary(summary, iteration)
                self.history_.append(iteration_loss)
