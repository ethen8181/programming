...
class VBPR(BaseEstimator):
    ...
    """
    def __init__(self, img_features, learning_rate = 0.01, n_factors = 15, 
                 n_img_factors = 10, n_iters = 10, n_batch_size = 2000, 
                 reg = 0.01, reg_img = 0.01, seed = 1234, verbose = True):
        self.reg = reg
        self.seed = seed
        ...
        self.n_img_factors = n_img_factors
        self.learning_rate = learning_rate
        self.img_features = img_features ## ? parameters or input data
    ...
    def fit(self, ratings):
        ...
        with tf.Session() as sess:
            ...
            self.user_img_factors = sess.run(self.user_img_factors)
            self.img_embedding = sess.run(self.img_embedding)
            self.img_bias = sess.run(self.img_bias)
    ...
    def _build_graph(self, n_users, n_items):
        ...
        n_img_features = self.img_features.shape[1]
        ...
        user_img_init = tf.truncated_normal((n_users, self.n_img_factors), seed = self.seed)
        img_bias_init = tf.truncated_normal((n_users, n_img_features), seed = self.seed)
        img_embedding_init = tf.truncated_normal((n_img_features, self.n_img_factors), seed = self.seed)
        ...
        self.user_img_factors = tf.Variable(user_img_init, name = 'user_img_factors')
        self.img_embedding = tf.Variable(img_embedding_init, name = 'img_embedding')
        self.img_bias = tf.Variable(img_bias_init, name = 'img_bias')
        img_features = tf.Variable(self.img_features, name = 'img_features')

        # the input data is the sampled batch of users index,
        # its corresponding positive and negative items' index and
        ...
        img_bias_u = tf.gather(self.img_bias, self._slice_u)
        img_feature_i = tf.transpose( tf.gather(img_features, self._slice_i) )
        img_feature_j = tf.transpose( tf.gather(img_features, self._slice_j) )

        # decompose the estimator, compute the difference between
        # the score of the positive items i and negative items j
        temp_img_uij = tf.matmul(tf.transpose(self.img_embedding), img_feature_i - img_feature_j)
        img_uij = tf.diag_part( tf.matmul(img_u, temp_img_uij) )
        r_uij = tf.diag_part( tf.matmul(user_u, tf.transpose(item_i - item_j)) )
        img_bias_uij = tf.diag_part( tf.matmul(img_bias_u, img_feature_i - img_feature_j) )
        diff = bias_i - bias_j + r_uij + img_uij + img_bias_uij
        ...
        cost_img = self.reg_img * ( tf.reduce_sum(self.img_embedding ** 2) + tf.reduce_sum(img_u ** 2) ) + tf.reduce_sum(img_bias_u ** 2)
        
        output = tf.clip_by_value(tf.nn.sigmoid(diff), 1e-10, 1.0)
        cost_uij = tf.reduce_sum( tf.log(output) )
        self._total_cost = cost_u + cost_i + cost_j + cost_img - cost_uij
    ...
    def _get_prediction(self):
        ...
        temp = self.user_img_factors.dot(self.img_embedding.T)
        self._pred += temp.dot(self.img_features.T)
        self._pred += self.img_bias.dot(self.img_features.T)
        return self
    ...
    def _predict_user(self, user):
        ...
        temp = self.user_img_factors[user].dot(self.img_embedding.T)
        user_pred += temp.dot(self.img_features.T)
        user_pred += self.img_bias[user].dot(self.img_features.T)
        return user_pred
